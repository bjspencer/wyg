...
IMPLICATIONS = {
    ...
    ('Averages 25+ PPG?', 1): [('Averages 20+ PPG?', 1), ('Averages 15+ PPG?', 1), ('Averages 10+ PPG?', 1)],
    ('Averages 20+ PPG?', 1): [('Averages 15+ PPG?', 1), ('Averages 10+ PPG?', 1)],
    ('Averages 15+ PPG?', 1): [('Averages 10+ PPG?', 1)],
    ('Averages 10+ PPG?', 0): [('Averages 15+ PPG?', 0), ('Averages 20+ PPG?', 0), ('Averages 25+ PPG?', 0)],
    ('Averages 15+ PPG?', 0): [('Averages 20+ PPG?', 0), ('Averages 25+ PPG?', 0)],
    ('Averages 20+ PPG?', 0): [('Averages 25+ PPG?', 0)],
    ('Averages 10+ RPG?', 1): [('Averages 8+ RPG?', 1), ('Averages 5+ RPG?', 1)],
    ('Averages 8+ RPG?', 1):  [('Averages 5+ RPG?', 1)],
    ('Averages 5+ RPG?', 0):  [('Averages 8+ RPG?', 0), ('Averages 10+ RPG?', 0)],
    ('Averages 8+ RPG?', 0):  [('Averages 10+ RPG?', 0)],
    ('Averages 7+ APG?', 1):  [('Averages 5+ APG?', 1), ('Averages 4+ APG?', 1)],
    ('Averages 5+ APG?', 1):  [('Averages 4+ APG?', 1)],
    ('Averages 4+ APG?', 0):  [('Averages 5+ APG?', 0), ('Averages 7+ APG?', 0)],
    ('Averages 5+ APG?', 0):  [('Averages 7+ APG?', 0)],
    ('Averages 1.5+ SPG?', 1): [('Averages 1+ SPG?', 1)],
    ('Averages 1+ SPG?', 0):   [('Averages 1.5+ SPG?', 0)],
    ('Averages 1.5+ BPG?', 1): [('Averages 1+ BPG?', 1)],
    ('Averages 1+ BPG?', 0):   [('Averages 1.5+ BPG?', 0)],
    ('Averages 3+ 3PM?', 1):  [('Averages 1+ 3PM?', 1)],
    ('Averages 1+ 3PM?', 0):  [('Averages 3+ 3PM?', 0)],
    ('Averages 30+ min per game?', 1): [('Averages 20+ min per game?', 1)],
    ('Averages 20+ min per game?', 0): [('Averages 30+ min per game?', 0)],
    ...
    ('Shoots 40%+ from three?', 1):         [('Shoots 35%+ from three?', 1)],
    ('Shoots 35%+ from three?', 0):         [('Shoots 40%+ from three?', 0)],
...
def load_dataset():
    ...
    for _, row in totals_df.iterrows():
        ...
        rows.append({
            ...
            'PLAYER_ID': pid,
            'Averages 25+ PPG?': int(ppg >= 25), 'Averages 20+ PPG?': int(ppg >= 20),
            'Averages 15+ PPG?': int(ppg >= 15), 'Averages 10+ PPG?': int(ppg >= 10),
            'Averages 10+ RPG?': int(rpg >= 10), 'Averages 5+ RPG?': int(rpg >= 5),
            'Averages 7+ APG?': int(apg >= 7), 'Averages 4+ APG?': int(apg >= 4),
            'Averages 1.5+ SPG?': int(row['STL']/gp >= 1.5), 'Averages 1+ SPG?': int(row['STL']/gp >= 1),
            'Averages 1.5+ BPG?': int(row['BLK']/gp >= 1.5), 'Averages 1+ BPG?': int(row['BLK']/gp >= 1),
            'Averages 3+ 3PM?': int(row['FG3M']/gp >= 3), 'Averages 1+ 3PM?': int(row['FG3M']/gp >= 1),
            'Averages 30+ min per game?': int(row['MIN']/gp >= 30), 'Averages 20+ min per game?': int(row['MIN']/gp >= 20),
            'Shoots 50%+ from the field?': int(row['FG_PCT'] >= 0.50),
            'Shoots 35%+ from three?': int(row['FG3_PCT'] >= 0.35),
            'Shoots 80%+ from the line?': int(row['FT_PCT'] >= 0.80),
            'Averages 3+ offensive rebounds?': int(row['OREB']/gp >= 3),
            'Averages 3+ turnovers?': int(row['TOV']/gp >= 3),
